...
class WalletEncryptionTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if not self.options.usecli: # can't be done with the test framework for cli since subprocess.Popen doesn't allow null characters
            ...
            assert_raises_rpc_error(-4, "The wallet passphrase entered was incorrect", self.nodes[0].walletpassphrasechange, passphrase_with_nulls.partition("\0")[0], "abc")
            ...
            assert_raises_rpc_error(-4, "The old wallet passphrase entered is incorrect or the new master key could not be written to the wallet database", self.nodes[0].walletpassphrasechange, passphrase_with_nulls + "\0", "abc")
