...
class WalletMigrationTest(BitcoinTestFramework):
    ...
    def test_relative_path_wallet_failure(self):
        self.log.info("Test failure during migration of relative path wallet")
        ...
        wallet_path = self.master_node.wallets_path / wallet_name / "wallet.dat"
        shutil.move(self.old_node.wallets_path / wallet_name / "wallet.dat", wallet_path)

        # Create wallet directory with the watch-only name and a wallet file.
        # Because the wallet dir exists, this will cause migration to fail.
        watch_only_dir = self.master_node.wallets_path / ".." / "_watchonly"
        os.mkdir(watch_only_dir)
        shutil.copyfile(wallet_path, watch_only_dir / "wallet.dat")
        ...
        with open(self.master_node.wallets_path / wallet_name / "wallet.dat", "rb") as f:
            data = f.read(16)
        ...
        backup_path = os.path.join(self.master_node.wallets_path, wallet_name, f"{backup_prefix}_{mocked_time}.legacy.bak")
    ...
    def test_absolute_path_wallet_failure(self):
        ...
        mocked_time = int(time.time())
        self.master_node.setmocktime(mocked_time)
        assert_raises_rpc_error(-4, "Wallet file creation failed: Failed to create database path", self.master_node.migratewallet, absolute_path)
        self.master_node.setmocktime(0)

        # Migration will fail to restore the original unnamed wallet
        # Verify the original wallet was not deleted
        absolute_wallet_path = os.path.join(absolute_path, "wallet.dat")
        assert os.path.exists(absolute_wallet_path)
        # And verify it is now a SQLite wallet
        with open(absolute_wallet_path, 'rb') as f:
            data = f.read(16)
            _, _, magic = struct.unpack("QII", data)
            assert_equal(magic, BTREE_MAGIC)
        # Verify the watchonly wallet was not deleted
        ...
        backup_path = os.path.join(absolute_path, f"{backup_prefix}_{mocked_time}.legacy.bak")
        assert os.path.exists(backup_path)
