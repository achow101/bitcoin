...
class WalletMigrationTest(BitcoinTestFramework):
    ...
    def test_absolute_path_wallet_failure(self):
        ...
        mocked_time = int(time.time())
        self.master_node.setmocktime(mocked_time)
        assert_raises_rpc_error(-1, "filesystem error: cannot copy file", self.master_node.migratewallet, absolute_path)
        self.master_node.setmocktime(0)
        ...
        with open(os.path.join(absolute_path, "wallet.dat"), 'rb') as f:
            file_magic = f.read(16)
            assert_equal(file_magic, b'SQLite format 3\x00')
        # Verify the watchonly wallet was not deleted
        assert os.path.exists(watch_only_dir / "wallet.dat")
        ...
        backup_path = os.path.join(self.master_node.datadir_path, f"{backup_prefix}_{mocked_time}.legacy.bak")
        assert os.path.exists(backup_path)
