...
class WalletMigrationTest(BitcoinTestFramework):
    ...
    def test_default_wallet_failure(self):
        ...
        mocked_time = int(time.time())
        self.nodes[0].setmocktime(mocked_time)
        assert_raises_rpc_error(-1, "filesystem error: cannot copy file: File exists", self.nodes[0].migratewallet, "")
        self.nodes[0].setmocktime(0)
    ...
    def test_relative_path_wallet_failure(self):
        ...
        wallet_path = self.nodes[0].wallets_path / wallet_name / "wallet.dat"
        ...
        with open(self.nodes[0].wallets_path / wallet_name / "wallet.dat", "rb") as f:
            data = f.read(16)
        ...
        # Check backup file exists.
        backup_path = os.path.join(self.nodes[0].datadir_path, f"_{mocked_time}.legacy.bak")
        assert os.path.exists(backup_path)
    ...
    def unsynced_wallet_on_pruned_node_fails(self):
        ...
        mocked_time = int(time.time())
        self.nodes[0].setmocktime(mocked_time)
        assert_raises_rpc_error(-4, "last wallet synchronisation goes beyond pruned data. You need to -reindex (download the whole blockchain again in case of pruned node)", self.nodes[0].migratewallet, wallet_name=wallet_name)
        self.nodes[0].setmocktime(0)

        # The wallet will not have been restored, and the backup will be in the wallets directory.
        backup_path = self.nodes[0].wallets_path / f"{wallet_name}_{mocked_time}.legacy.bak"
        assert backup_path.exists()
