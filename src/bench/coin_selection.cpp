...
static void CoinSelection(benchmark::Bench& bench)
{
    ...
    for (const auto& wtx : wtxs) {
        ...
        coins.emplace_back(wtx->tx->vout[0], COutPoint(wtx->GetHash(), 0), wtx->IsFromMe(ISMINE_ALL), wtx->GetTxTime(), wtx->m_confirm, true);
...
static void add_coin(const CAmount& nValue, int nInput, std::vector<OutputGroup>& set)
{
    ...
    set.emplace_back();
    COutput output(wtx->tx->vout[nInput], COutPoint(wtx->GetHash(), nInput), true, wtx->GetTxTime(), wtx->m_confirm, true);
