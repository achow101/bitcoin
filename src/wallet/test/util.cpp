...
std::unique_ptr<WalletDatabase> DuplicateMockDatabase(WalletDatabase& database)
{
    ...
    while (true) {
        ...
        DatabaseCursor::Status status = txs_cursor->NextTx(txid, ser_tx, comment, comment_to, replaces, replaced_by, timesmart, order_pos, messages, payment_requests, state_type, state_data);
        Assert(status != DatabaseCursor::Status::FAIL);
        if (status == DatabaseCursor::Status::DONE) break;

        batch_new->WriteTx(txid, ser_tx, comment, comment_to, replaces, replaced_by, timesmart, order_pos, messages, payment_requests, state_type, state_data);
    }
