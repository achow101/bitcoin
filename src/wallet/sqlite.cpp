...
template <typename T>
concept ColumnSpanBlob = requires(T a, std::span<typename T::value_type> s)
{
    T(s);
};
...
public:
    ...
    std::optional<T> Column(int col)
    {
        if constexpr (std::integral<T> && sizeof(T) <= 4) {
            ...
            const char* text = (const char*)sqlite3_column_text(m_stmt, 0);
            // Strings may be non-null and empty, so distinguish between such strings
            // and a NULL column value
            if (!text) return std::nullopt;
            size_t size = sqlite3_column_bytes(m_stmt, col);
            std::string str_text(text, size);
            return str_text;
        } else if constexpr (ColumnBlob<T>) {
            // We do not need to distinguish between NULL and empty blobs since
            // NULL is written as an empty blob.
            // This behavior is only preserved for backwards compatibility
            return T(
            ...
            const typename T::value_type* data = reinterpret_cast<const T::value_type*>(sqlite3_column_blob(m_stmt, col));
...
void SQLiteBatch::Close()
{
    ...
    const std::vector<std::pair<std::unique_ptr<SQLiteStatement>*, const char*>> statements{
        {&m_read_stmt, "read"},
...
bool SQLiteBatch::ExecDeleteStatement(SQLiteStatement* stmt, std::span<const std::byte> blob)
{
    if (!m_database.m_db) return false;
    assert(stmt);
...
{
    return ExecDeleteStatement(m_delete_stmt.get(), key);
}

bool SQLiteBatch::ErasePrefix(std::span<const std::byte> prefix)
{
    return ExecDeleteStatement(m_delete_prefix_stmt.get(), prefix);
}
...
bool SQLiteBatch::UpdateTxReplaces(const Txid& txid, const Txid& replaces)
{
    ...
    return ExecStatement(m_update_tx_replaces_stmt.get());
}
