...
private:
    ...
    std::unordered_map<uint64_t, CPubKey> m_sp_labels GUARDED_BY(cs_sp_man);
    //! The label for change outputs
    std::pair<uint256, CPubKey> m_change_label GUARDED_BY(cs_sp_man);
    ...
    void SetupLabels(int64_t labels_size);
