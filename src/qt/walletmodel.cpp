...
void WalletModel::updateAddressBook(const QString &address, const QString &label,
        bool isMine, const std::optional<interfaces::Wallet::AddressPurpose>& purpose, int status)
{
    ...
    {
        {
            ...
            std::string strLabel = rcp.label.toStdString();
            {
                ...
                {
                    m_wallet->setAddressBook(dest, strLabel, interfaces::Wallet::AddressPurpose::SEND);
                }
                else if (name != strLabel)
                {
                    m_wallet->setAddressBook(dest, strLabel, std::nullopt);
                }
            }
        }
        ...
        const std::optional<interfaces::Wallet::AddressPurpose>& purpose, ChangeType status)
{
    ...
    bool invoked = QMetaObject::invokeMethod(walletmodel, "updateAddressBook",
                              ...
                              Q_ARG(bool, isMine),
                              Q_ARG(std::optional<interfaces::Wallet::AddressPurpose>, purpose),
                              Q_ARG(int, status));
    assert(invoked);
}
