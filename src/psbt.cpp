...
bool PartiallySignedTransaction::Merge(const PartiallySignedTransaction& psbt)
{
    ...
    if (m_tx_modifiable != std::nullopt || psbt.m_tx_modifiable != std::nullopt) {
        m_tx_modifiable = this_modifiable;
    }
...
Txid PartiallySignedTransaction::GetUniqueID() const
{
    ...
    // Note: Does not use GetUnsignedTx() as it is allowed to fail
    CMutableTransaction mtx;
    mtx.version = tx_version;
